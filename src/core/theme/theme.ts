...
export const THEMES: Record<AppearanceMode, ThemeColors> = {
  ...
  orbit: {
    background: '#08090B',
    backgroundAlt: '#08090B',
    surface: '#141518',
    surfaceElevated: '#1A1B20',
    card: 'rgba(20,21,24,0.88)',
    ...
    profit: '#00C853',
    profitSoft: 'rgba(0,200,83,0.14)',
    loss: '#FF5252',
    lossSoft: 'rgba(255,82,82,0.14)',
    warning: '#F59E0B',
    overlay: 'rgba(5,5,8,0.74)',
    fieldBackground: 'rgba(255,255,255,0.04)',
    chipBackground: 'rgba(255,255,255,0.05)',
    cardGradient: ['rgba(17,19,24,0.94)', 'rgba(8,9,11,0.96)'],
    highlightGradient: ['rgba(255,255,255,0.08)', 'rgba(12,13,16,0.96)'],
...
export const ORBITX_COLORS = {
  ...
  textMuted: 'rgba(161,161,170,0.65)',
  purple: '#7B3FE4',
  purpleSoft: '#8B5CF6',
  green: '#00C853',
  cyan: '#00E5FF',
...
export function withOpacity(hexColor: string, alpha: number) {
  ...
  const rgbMatch = hexColor
    .trim()
    .match(/^rgba?\(\s*([\d.]+)\s*,\s*([\d.]+)\s*,\s*([\d.]+)(?:\s*,\s*([\d.]+))?\s*\)$/i);

  if (rgbMatch) {
    const [, red, green, blue, existingAlpha] = rgbMatch;
    const baseAlpha = existingAlpha === undefined ? 1 : Number(existingAlpha);
    const nextAlpha = Number.isFinite(baseAlpha) ? clampedAlpha * baseAlpha : clampedAlpha;
    return `rgba(${Number(red)}, ${Number(green)}, ${Number(blue)}, ${Math.min(Math.max(nextAlpha, 0), 1)})`;
  }

  if (!hexColor.trim().startsWith('#')) {
    return hexColor;
  }
  ...
  const normalized =
    hex.length === 3
      ? hex
          ...
          .join('')
      : hex;
  const opacity = Math.round(clampedAlpha * 255)
    .toString(16)
    .padStart(2, '0');
