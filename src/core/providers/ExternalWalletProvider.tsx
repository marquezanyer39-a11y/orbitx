...
import {
  ...
  type WalletConnectRequestProvider,
  getWalletConnectRuntimeModules,
...
function ExternalWalletRuntimeProvider({ children }: { children: ReactNode }) {
  ...
  const { provider: eip155Provider, providerType } = useProvider();
      ...
      void setExternalWalletConnection(
        {
          ...
          lastError: hasSupportedNetwork
            ? undefined
            : 'La red actual no es compatible con QVEX para operaciones EVM.',
        },
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      const message = normalizeWalletErrorMessage(
        error,
        'No se pudo abrir el selector de wallets externas.',
      );
      setBusyAction(null);
      updateExternalWalletState({
        status: 'error',
  ...
  }, [open, showToast, updateExternalWalletState]);
  ...
  const switchToNetwork = useCallback(
    async (network: SupportedNetwork) => {
      if (network === 'solana') {
        return {
          ok: false,
          message: 'La wallet externa actual solo maneja redes EVM dentro de QVEX.',
        };
      }
  ...
  const value = useMemo<ExternalWalletContextValue>(
    () => ({
      ...
      eip155Provider: isConnected && providerType === 'eip155' ? eip155Provider : undefined,
...
export function ExternalWalletProvider({
  ...
  if (!enableRuntime || !FEATURE_STATUS.web3.allowWalletConnect) {
    return (
      <ExternalWalletContext.Provider
        value={{
          ...defaultContextValue,
  ...
  const walletConnectAppKit = getWalletConnectAppKit();
